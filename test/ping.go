...
/*func main() {
	...
	resTime := ping(args[0], argsmap)
	//fmt.Printf("resTime: %dms\n", resTime)
...
func ping(host string, count, size int, timeout int64, neverstop bool, rtt chan int, lossRate chan float32, jump chan int) {

	//cname, _ := net.LookupCNAME(host)
	/*starttime := time.Now()
	conn, err := net.DialTimeout("ip4:icmp", host, time.Duration(timeout*1000*1000))
	if(err != nil){
		fmt.Println("connection dial failed!")
		rtt <- 0
		lossRate <- 0
		return
	}*/
	...
	for count > 0 || neverstop {
		...
		if err != nil || receive[ECHO_REPLY_HEAD_LEN+4] != msg[4] || receive[ECHO_REPLY_HEAD_LEN+5] != msg[5] || receive[ECHO_REPLY_HEAD_LEN+6] != msg[6] || receive[ECHO_REPLY_HEAD_LEN+7] != msg[7] || endduration >= int(timeout) || receive[ECHO_REPLY_HEAD_LEN] == 11 {
			...
			recvN++
			ttl := int(receive[8])
			for _, maxTTL := range []int{32, 64, 128, 256}{
				jumpNum = maxTTL - ttl
				if jumpNum > 0{
					break
				}
			}
			//fmt.Println("正在 Ping " + host + " 具有 32 字节的数据:")
			//fmt.Print(ttl, " ")
	...
	if(recvN == 0){
		...
		rtt <- sumT/recvN
		lossRate <- float32(sendN - recvN)/float32(sendN)
		jump <- jumpNum
	}
